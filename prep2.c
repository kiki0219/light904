...
int main(void)
{
	...
	// /*设置对应2个参数的C变量，用来保存数据 */
	// char name[30];
	// char password[30];//密码（未加密）
	// bzero(param,sizeof(param));//清空

	// printf("username:\n");		//发送给客户端
	// scanf("%s",name);           //接收客户端
	// printf("password:\n" );     //发送给客户端
	// scanf("%s",password);       //接收客户端
...
//mysql> truncate light;
	...
	/*设置对应2个参数的C变量，用来保存数据 */
	char uname[30];
	char upassword[30];//密码（未加密）
	bzero(param,sizeof(param));//清空
	printf("username:\n");		//发送给客户端
	scanf("%s",uname);           //接收客户端
	printf("password:\n" );     //发送给客户端
	scanf("%s",upassword);       //接收客户端
	...
	/*将服务器发来的值显示出来 */
	while(mysql_stmt_fetch(statement)!=MYSQL_NO_DATA)
	{
		printf("%s\t%s\n",name,password);
	}
