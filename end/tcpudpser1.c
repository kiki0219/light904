            ...
            if(FD_ISSET(udp_fd,&read_fds))
            {
                ...
                if(head!=NULL)
                {
                    ...
                    while(curr!=NULL)
                    {
                        ...
                        {
                            //printf("%s\t%d\n",inet_ntoa(curr->info.client_addr.sin_addr),ntohs(curr->info.client_addr.sin_port));
                            //curr->info.client_addr.sin_addr.s_addr=inet_addr("inet_ntoa(curr->info.client_addr.sin_addr)");
                            //curr->info.client_addr_len=sizeof(curr->info.client_addr);
                            //sendto(udp_fd,&info,sizeof(info),0,(struct sockaddr*)&curr->info.client_addr,curr->info.client_addr_len);
                            send(curr->info.fd,&info,sizeof(info),0);
                            printf("%s\t%s\n",info.dest_client_id,info.msg);
                            break;
                        }
                        curr=curr->next;
                    }
